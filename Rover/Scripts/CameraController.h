...
namespace mrs
{
    ...
    private:
        void HandleKeyBoardInput(float dt)
        {
            ...
            {
				camera_transform.position += _camera->GetUp() * _movement_speed * dt;
			}
            ...
            {
				camera_transform.position += _camera->GetFront() * _movement_speed * dt;
			}
            ...
            {
				camera_transform.position += _camera->GetRight() * _movement_speed * dt;
			}
        ...
        void HandleMouseInput(float dt)
        {
            ...
            if(int x = Input::GetAxis('x'))
            {
                _camera->GetYaw() += (x * _mouse_sensitivity);
            }

            if(int y = Input::GetAxis('y'))
            {
